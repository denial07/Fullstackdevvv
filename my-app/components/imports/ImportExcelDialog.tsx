...
export type ImportExcelDialogProps = {
    /** Logical entity name understood by your backend (e.g., "Shipment", "Product"). */
    entity: string;
    /** React element that opens the dialog. Example: <Button>Import Excel</Button> */
    trigger?: React.ReactNode;
    /** Override endpoints if different. */
    inspectUrl?: string; // default: "/api/import/inspect"
    commitUrl?: string; // default: "/api/import/commit"
...
export default function ImportExcelDialog({
    ...
    const [mappingEdits, setMappingEdits] = useState<Record<string, string>>({}); // incoming -> mapTo
    const [dupDecisions, setDupDecisions] = useState<Record<number, "insert" | "skip">>({}); // index in review
    ...
    async function handleRunInspect() {
        ...
        try {
            ...
            // dryRun toggle is for UX; server may ignore
            ...
            <DialogContent className="max-w-4xl">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        <FileSpreadsheet className="h-5 w-5" /> Import {entity}s from Excel
                    </DialogTitle>
                    <DialogDescription>
                        Upload an .xlsx/.xls file. We'll do a dry-run to detect columns, suggest mappings (95% name & type confidence), and check duplicates.
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <Tabs defaultValue="mapping" className="w-full">
                                            ...
                                            {inspect.mapping.map((m) => {
                                                ...
                                                return (
                                                    <TableRow key={m.incoming}>
                                                        ...
                                                        <TableCell>
                                                            {needs ? (
                                                                <Badge variant="destructive" className="flex items-center gap-1"><AlertTriangle className="h-3 w-3" /> Needs confirm</Badge>
                                                            ) : (
                                                                <Badge variant="secondary" className="flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> Auto</Badge>
                                                            )}
                                                        </TableCell>
                                                    </TableRow>
                                                );
                            ...
                            <TabsContent value="duplicates" className="space-y-3">
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {inspect.duplicates.review.length === 0 && (
                                                <TableRow>
                                                    <TableCell colSpan={3} className="text-center text-sm text-muted-foreground">No uncertain duplicates detected.</TableCell>
                                                </TableRow>
                                            )}
                                            {inspect.duplicates.review.map((d, idx) => {
                                                ...
                                                return (
                                                    <TableRow key={idx}>
                                                        <TableCell>
                                                            <div className="flex items-center gap-2">
                                                                <Button type="button" variant={action === "insert" ? "default" : "outline"} size="sm" onClick={() => setDupDecisions((p) => ({ ...p, [idx]: "insert" }))}>Insert</Button>
                                                                <Button type="button" variant={action === "skip" ? "default" : "outline"} size="sm" onClick={() => setDupDecisions((p) => ({ ...p, [idx]: "skip" }))}>Skip</Button>
                                                            </div>
                                                        </TableCell>
                                                        <TableCell>
                                ...
                                {inspect.duplicates.autoInsert.length > 0 && (
                                    <p className="text-xs text-muted-foreground">{inspect.duplicates.autoInsert.length} rows will be auto-inserted (≥90% confidence NOT duplicate).</p>
                                )}
                            </TabsContent>
                ...
                <DialogFooter className="flex items-center justify-between">
                    <div className="text-xs text-muted-foreground">
                        Tip: We only auto-map when both name and type confidence ≥95%.
                    </div>
                    <div className="flex gap-2">
                        <Button variant="outline" onClick={() => { resetDialog(); setOpen(false); }}>Close</Button>
...
function ConfidenceBadge({ value, threshold, inverse = false }: { value: number; threshold: number; inverse?: boolean }) {
    // inverse=true: lower is better (e.g., probabilityDuplicate), so flip logic for color
